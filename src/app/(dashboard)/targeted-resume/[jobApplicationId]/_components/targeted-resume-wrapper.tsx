...
import { DbTargetedResumeAnalysis, JobApplication } from "@/types/database"

import { saveResumeAnalysis } from "../_actions/targeted-resume-actions"
import { tailorSchema } from "../_schema/tailor-schema"

import TargetedResumeForm from "./targeted-resume-form"
import TargetedResumeResult from "./targeted-resume-result"
import TargetedResumeViewer from "./targeted-resume-viewer"
...
interface TargetedResumeWrapperProps {
  jobApplicationData: JobApplication
  previousAnalysis?: DbTargetedResumeAnalysis | null
}

export default function TargetedResumeWrapper({
  jobApplicationData,
  previousAnalysis,
}: TargetedResumeWrapperProps) {
  const [savedAnalysis, setSavedAnalysis] =
    useState<DbTargetedResumeAnalysis | null>(previousAnalysis || null)
      ...
      if (result.object && jobApplicationData.resumes) {
        ...
        try {
          ...
          if (saveResult.status === "success") {
            ...
            if (saveResult.data) {
              setSavedAnalysis(saveResult.data)
            }
